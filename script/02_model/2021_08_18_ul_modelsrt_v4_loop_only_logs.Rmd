---
title: "model from cored las wells, just logs"
...
We will use to train the system the logs of the wells with core data, this assumes that the SRT interpretation is more reliable on this wells.
...
to_train <-
  all_logs %>% 
  rename(srt = SRT_OPCO) %>% 
    select(logs, srt, DEPT, WELL, main_zone) %>% 
    drop_na()
...
for (i in 2:n) {
  ...
  for (j in 1:loop) {
    ...
    if (kh_roc_auc > kh_roc_auc_max) {
      ...
      print(cat( "kh ", count ,  kh_roc_auc_max, kh_the_logs))
    ...
    if (lk_roc_auc > lk_roc_auc_max) {
      ...
      print(cat( "lk ",count ,  lk_roc_auc_max, lk_the_logs))
