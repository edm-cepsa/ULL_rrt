...
logs <- c("XGR", "XSRES", "XPHIT", "res_diff_log", "res_diff_lin", "log_XPERM")
...
xdata <- read_rds("../../clean_data/all_logs_for_model.rds") %>% 
  ...
  select("XGR"    ,   "XSRES"   ,  "XPHIT"  ,   "res_diff_log" ,"res_diff_lin", "log_XPERM", "main_zone" , "Well", "MD" ) %>% 
  drop_na()
...
temp_df <- xdata %>% 
  ...
  rename(DEPT = MD, 
         WELL = Well) %>% 
  group_by(WELL) %>% 
  do(write_las(., name = unique(.$WELL), do = TRUE))
...
logs <- c( "XGR", "XSRES", "XPHIT", "res_diff_lin", "log_XPERM", "log_XDRES" )
...
xdata <- read_rds("../../clean_data/all_logs_for_model.rds") %>% 
  ...
  select("XGR", "XSRES", "XPHIT", "res_diff_lin", "log_XPERM", "log_XDRES" , "main_zone" , "Well", "MD" ) %>% 
  drop_na()
